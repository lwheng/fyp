...
if __name__ == "__main__":
  ...
  for f in second_tier_keys:
    #f = "C04-1201==>J03-3001" # REMOVE AFTER DEBUG
    info = f.split("==>")
    citing = info[0]
    cited = info[1]
    ...
    for context_id in f_hash.keys():
      ...
      x = feature_extractor.extract_feature_2nd_tier_baseline(f, c, citing_col, doms[hash_key][1], doms[hash_key][3])
  ...
  ## Write out X
  pickle.dump(X, open(os.path.join(path_pickles, 'X_2nd_Tier_Baseline.pickle'),'wb'))
  ## Write out y
  pickle.dump(y, open(os.path.join(path_pickles, 'y_2nd_Tier_Baseline.pickle'),'wb'))
  ## Write out the Model
  pickle.dump(model, open(os.path.join(path_pickles, 'Model_2nd_Tier_Baseline.pickle'),'wb'))
