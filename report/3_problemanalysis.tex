...
We abstracted away the problem of locating the in-line citations in a paper, and reduced the problem to only determining the type of a citation and its location. To solve the problem of locating the in-line citations, we utilized the open-source ParsCit system developed by \outcite{parscit}. Conveniently, ParsCit identifies the citing sentence, together with its citing context.
